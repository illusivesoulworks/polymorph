...
public class RecipeSelectorGui<I extends IInventory, R extends IRecipe<I>> extends AbstractGui
  ...
  public RecipeSelectorGui(int x, int y, int xOffset, int yOffset, I inventory,
                           Consumer<R> onSelect, ContainerScreen<?> screen) {
  ...
  public void highlightButton(String recipe) {
    this.outputWidgets.forEach(widget -> {
      String id = widget.getRecipe().getId().toString();
      widget.highlighted = id.equals(recipe);
    });
  }
  ...
  public List<RecipeOutputWidget<I, R>> getOutputWidgets() {
  ...
  public void setRecipes(List<R> recipes) {
    ...
    recipes.forEach(recipe -> this.outputWidgets
        .add(new RecipeOutputWidget<>(recipe, recipe.getCraftingResult(this.inventory))));
    this.updateButtonPositions();
  }
  ...
  public boolean mouseClicked(double mouseX, double mouseY, int button) {
    ...
    if (this.isActive()) {
      ...
      for (RecipeOutputWidget<I, R> widget : this.outputWidgets) {

        if (widget.mouseClicked(mouseX, mouseY, button)) {
          onSelect.accept(widget.getRecipe());
          return true;
        }
      }
    }
