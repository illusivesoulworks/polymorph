...
import top.theillusivec4.polymorph.mixin.core.AccessorCraftingInventory;
...
public class MixinHooks {
  ...
  public static <T extends Recipe<C>, C extends Inventory> Optional<T> getResult(
    ...
    if (inventory instanceof CraftingInventory craftingInventory) {
      ...
      if (((AccessorCraftingInventory) craftingInventory).getHandler() != player.currentScreenHandler) {
        send = false;
      }
    }
    ...
    if (recipes.isEmpty()) {
      ...
      if (player instanceof ServerPlayerEntity && send) {
